...
function getRange(url: URL) {
  ...
  if (range === "week") {
    const weekday = Number(new Intl.DateTimeFormat("en-US", { timeZone: TZ, weekday: "short" }).format(now) === "Sun" ? 0 : now.getDay())
...
export async function GET(request: Request) {
  try {
    ...
    for (const order of data || []) {
      ...
      const paymentLabel = group === "pix" ? "Pix" : group === "card" ? `Cartão ${installments > 1 ? `${installments}x` : "à vista"}` : group === "boleto" ? "Boleto" : group === "cash" ? "Dinheiro" : String(order.payment_method || "Outro")
      const shippingLabel = shipping === "bus" ? "Ônibus / excursão" : String(order.shipping_method || "Não informado")

      if (paid(order)) {
        movements.push({
          id: `${order.id}-sale`, date: raw.paid_at || order.created_at, orderNumber: String(order.order_number || order.external_id),
          customer: order.customer_name || "Cliente", type: "sale", paymentGroup: group, paymentLabel,
          shippingGroup: shipping, shippingLabel, installments, itemCount, productGross: subtotal, discount,
          productNet, freight, busFee: shipping === "bus" ? freight : 0, totalReceived: total,
          paymentFee, refund: 0, net: money(total - paymentFee), feeEstimated: estimated,
      ...
      if (refund > 0) {
        movements.push({
          id: `${order.id}-refund`, date: raw.refunded_at || order.updated_at || order.created_at,
          orderNumber: String(order.order_number || order.external_id), customer: order.customer_name || "Cliente",
          type: "refund", paymentGroup: group, paymentLabel, shippingGroup: shipping, shippingLabel,
          installments, itemCount: 0, productGross: 0, discount: 0, productNet: 0, freight: 0,
          busFee: 0, totalReceived: 0, paymentFee: 0, refund, net: -refund, feeEstimated: false,
          status: String(order.payment_status || "Reembolsado")
        })
      }
    }

    const summary = { orders: 0, items: 0, productGross: 0, discounts: 0, salesWithoutFreight: 0, freight: 0, busFees: 0, totalReceived: 0, paymentFees: 0, refunds: 0, net: 0, ticketWithoutFreight: 0, estimatedFeeOrders: 0 }
    const methods: any = {}
    for (const key of ["pix", "card", "boleto", "cash", "other"]) methods[key] = { orders: 0, received: 0, fees: 0, refunds: 0, net: 0 }
    ...
    for (const movement of movements) {
      ...
      if (movement.type === "sale") {
        summary.orders++; summary.items += movement.itemCount; summary.productGross += movement.productGross
        summary.discounts += movement.discount; summary.salesWithoutFreight += movement.productNet
        summary.freight += movement.freight; summary.busFees += movement.busFee; summary.totalReceived += movement.totalReceived
        summary.paymentFees += movement.paymentFee; if (movement.feeEstimated) summary.estimatedFeeOrders++
        methods[movement.paymentGroup].orders++; methods[movement.paymentGroup].received += movement.totalReceived
        methods[movement.paymentGroup].fees += movement.paymentFee
        if (movement.shippingGroup === "bus") {
          methods.bus.orders++; methods.bus.salesWithoutFreight += movement.productNet; methods.bus.charged += movement.busFee
          methods.bus.paymentFees += movement.paymentFee; methods.bus.net += movement.net
        }
        day.sales += movement.totalReceived; day.fees += movement.paymentFee
      } else {
        summary.refunds += movement.refund; methods[movement.paymentGroup].refunds += movement.refund; day.refunds += movement.refund
      }
      summary.net += movement.net; methods[movement.paymentGroup].net += movement.net; day.net += movement.net
      chartMap.set(key, day)
    }

    summary.ticketWithoutFreight = summary.orders ? money(summary.salesWithoutFreight / summary.orders) : 0
    for (const key of Object.keys(summary)) (summary as any)[key] = typeof (summary as any)[key] === "number" ? money((summary as any)[key]) : (summary as any)[key]
    for (const key of ["pix", "card", "boleto", "cash", "other"]) {
      methods[key].received = money(methods[key].received); methods[key].fees = money(methods[key].fees)
      methods[key].refunds = money(methods[key].refunds); methods[key].net = money(methods[key].net)
    }
    for (const key of ["salesWithoutFreight", "charged", "paymentFees", "net"]) methods.bus[key] = money(methods.bus[key])

    return Response.json({
      generatedAt: new Date().toISOString(), range: { key: range, label: `${from} a ${to}`, from, to },
      metrics: summary, methods, chart: Array.from(chartMap.values()).sort((a, b) => a.date.localeCompare(b.date)).map(item => ({ ...item, sales: money(item.sales), fees: money(item.fees), refunds: money(item.refunds), net: money(item.net) })),
      movements, feeConfig: { pixPercent: pixRate, cardPercent: cardRate, otherPercent: otherRate, estimatedFeeOrders: summary.estimatedFeeOrders }
    })
  } catch (error) {
